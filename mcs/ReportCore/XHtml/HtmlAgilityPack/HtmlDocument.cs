...
// https://github.com/alexandernyquist/HtmlAgilityPack
// Official site: http://htmlagilitypack.codeplex.com
...
namespace HtmlAgilityPack
{
	...
	public partial class HtmlDocument
	{
        ...
        public static HtmlDocument Xhtml(string html)
        {
            var xhtml = new HtmlDocument();
            xhtml.OptionWriteEmptyNodes = true;
            xhtml.OptionOutputAsXml = true;
            xhtml.OptionCheckSyntax = true;
            xhtml.OptionFixNestedTags = true;
            xhtml.OptionAutoCloseOnEnd = true;
            xhtml.OptionOutputOptimizeAttributeValues = false;

            xhtml.LoadHtml(html);
            xhtml.LoadHtml(xhtml.DocumentNode.WriteContentTo());

            return xhtml;
        }

        public object[] XElementContent()
        {
           if (DocumentNode == null)
               return null;

           object[] ret = null;
           Array.Resize(ref ret, DocumentNode.ChildNodes.Count);
           for (int i = 0; i < ret.Length; i++)
                ret[i] = XElement.Parse(DocumentNode.ChildNodes[i].OuterHtml);
		...
		internal Encoding GetOutEncoding()
		{
			...
			return _declaredencoding ?? (_streamencoding ?? 
                   (OptionDefaultStreamEncoding ?? UTF8Encoding.UTF8));
		}
