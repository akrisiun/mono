...
namespace IQToolkit.Data.Mapping
{
    ...
    public class AttributeMapping : AdvancedMapping
    {
        ...
        private MappingEntity GetEntity(Type elementType, string tableId, Type entityType)
        {
            ...
            if (entities.Count > 0 && !entities.TryGetValue(tableId, out entity))
            {
        ...
        private MappingEntity CreateEntity(Type elementType, string tableId, Type entityType)
        {
            ...
            IEnumerable<TableBaseAttribute> tableAttributes = null;

            if (rootTableId.Length > 0) {
                var mappingAttributes = this.GetMappingAttributes(rootTableId);
                tableAttributes = mappingAttributes.OfType<TableBaseAttribute>()
                    .OrderBy(ta => ta.Name);
                tableAttr = tableAttributes.OfType<TableAttribute>().FirstOrDefault();
                ...
                foreach (var attr in memberAttributes) {
                    ...
                    if (memberName.Contains('.')) // additional nested mappings
                    {
                        ...
                        if (members.Contains(memberName)) {
                            ...
                            "AttributeMapping: more than one mapping attribute specified for member '{0}' on type '{1}'", memberName, entityType.Name));
                        }
            ...
            if (rootTableId.Length <= 0 || mappingMembers.Count == 0) {

                tableAttr = new TableAttribute { EntityType = elementType, Name = tableId };
                tableAttributes = new TableBaseAttribute[] { tableAttr };

                PropertyInfo[] info = elementType.GetProperties();
                mappingMembers = new List<AttributeMappingMember>();
                foreach (var item in info) 
                {
                    MemberInfo m = item; 
                    // item.Attributes ??
                    MemberAttribute a = new ColumnAttribute { Name = item.Name };
                    if (item.CanWrite) {
                        var amm = new AttributeMappingMember(m, a, null);
                        mappingMembers.Add(amm);
                    }
                }
            }
        ...
        private MemberInfo FindMember(Type type, string path)
        {
            ...
            if (names.Length == 1 && names[0] == path) {
                object attr = new { attr = new TableAttribute { Name = path, EntityType = type } };
                member = attr.GetType().GetMember("attr").FirstOrDefault();
                return member;
            }
        ...
        public override string GetTableName(MappingEntity entity)
        {
            ...
            return table != null ? this.GetTableName(table) : entity.EntityType.Name;
        }
        ...
        class AttributeMappingMember
        {
            ...
            public AttributeMappingMember(PropertyInfo info, MemberAttribute attribute, Type elementType = null) { 
            
                this.attribute = attribute;
                this.member = info as MemberInfo;
                this.nested = null; // new AttributeMappingEntity 
            }
        ...
        class AttributeMappingEntity : MappingEntity
        {
            ...
            internal AttributeMappingEntity(Type elementType, string tableId, Type entityType, IEnumerable<TableBaseAttribute> attrs, IEnumerable<AttributeMappingMember> mappingMembers)
            {
                ...
                this.mappingMembers = mappingMembers.ToDictionary(mm => (mm.Column.Name ?? mm.Member?.Name));
            }
