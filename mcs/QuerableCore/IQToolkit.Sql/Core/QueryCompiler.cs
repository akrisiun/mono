            ...
            private Func<object[], object> GetInvoker()
            {
                if (this.fnQuery != null && this.invoker == null && !checkedForInvoker)
                {
                    ...
                    if (fnType.FullName.StartsWith("System.Func`"))
                    {
                        ...
                        MethodInfo method = this.GetType().GetTypeInfo().GetMethod("FastInvoke" + typeArgs.Length, BindingFlags.Public|BindingFlags.Instance);
                        if (method != null)
                        {
                            this.invoker = (Func<object[], object>)
                                WrapDelegate.CreateDelegateFunc<object[], object>(
                                typeof(Func<object[], object>), this, method.MakeGenericMethod(typeArgs));
                        }
                    }
                }
                return this.invoker;
