...
main (int argc, char* argv[]) {
	...
	// g_string_append monoeg_g_string_append
	g_string_append(s, "DEBUG");

	// gint 
	// https://linux.die.net/man/3/vfprintf
	g_printf("| Hello mono Test1: %s |", s->str);

	gchar  *dir = g_get_current_dir();
	g_printf("current dir: %s |\n", dir);

	debug = g_getenv("MONO_DEBUG");
	// g_setenv("MONO_DEBUG", "native-debugger-break", 1);
	
	if (argc < 2) {

		// fprintf (stderr, "Please provide an assembly to load\n");
		// return 1;
		// char test[] = "TestWeb.exe";

		// & ../bin/mono-sgen.exe 
		char test1[] = "--debug";
		char test3[] = "TestWeb.exe";

		argv[1] = &test3;
		argc = 2;
		argv[2] = &test1;

		char test4[] = "--mixed-mode";
		char test2[] = "--debugger-agent=transport=dt_socket,server=y,address=127.0.0.1:55555";
		argv[1] = &test1;
		argv[2] = &test2;
		argv[3] = &test3;
		argv[4] = &test4;
		argc = 4;
		g_printf("args: %s %s %s %s", argv[1], argv[2], argv[3], argv[4]);
	}

	file = argv [1];
	
	// g_find_program_in_path
	findPath = monoeg_g_find_program_in_path(file);
	/*
	...
	if (argc == 2) {
		...
		mono_main_with_options(argc - 1, argv + 1);
	}
